...
@section('content')
<?php $page_navigation_no='workbench'; ?>
...
try{
?>
    <div class="table-section" id="workbench_details">
		<div class="container-fluid" style="margin-bottom: 10px;margin-top: 20px;">
			<div class="row">
				...
				<div class="col-md-5">
					<input type='hidden' id="fin_path" value='workbench'/>
					<?php
				...
				<div class="col-md-12 is-tab-content">
        			<ul class="nav nav-tabs am-menu" style="margin-bottom:10px;">
						<li class="active nav-item">
							<a class="nav-link" data-toggle="tab" href="#workbench_tocheck">To check</a>
						</li>
						<li class="nav-item">
							<a class="nav-link" data-toggle="tab" href="#workbench_checked">Checked</a>
						</li>
            		</ul>
    			</div>
			</div>
			<div class="row col-md-2" style="float:right">
				<input id="workbench_table_filter" class="dataTables_custom_filter" placeholder="Search" class="form-control">
			</div>
		</div>
		<div class="container-fluid table-content">
			<div class="row tab-content travel-request-table">
				<div id="workbench_tocheck" class="col-md-12 tab-pane active">
					<table id="workbench_tocheck_table">
						<thead>
							<tr>	
								<th>Request id</th>
								<th>Request type</th>
						...
						<tbody>	
						@foreach($workbench_tocheck as $requests)
							<tr>
								<td>
								...
								<td>
									<a href="{{ $requests->module == "MOD_03" ? '/visa_request/'.Crypt::encrypt($requests->id) : '/request_full_details/'.Crypt::encrypt($requests->id) }}""><img src="/images/action.svg" data-toggle="tooltip" class="action_image" title="To process"></a>
								</td>
							</tr>
						@endforeach	
						</tbody>	
				...
				<div id="workbench_checked" class="col-md-12 tab-pane fade">
					<table id="workbench_checked_table">
						<thead>
							<tr>	
						...
						<tbody>	
							@foreach($workbench_checked as $requests)
							<tr>
								<td>
								<a href="{{ $requests->module == "MOD_03" ? '/visa_request/'.Crypt::encrypt($requests->id) : '/request_full_details/'.Crypt::encrypt($requests->id) }}">
								<img src="/images/status/{{$icons[$requests->status_id]}}" class="status_img " title="{{$requests->status_name}}" data-toggle="tooltip">
